...
void computeORBDesc(const cv::Mat &image, vector<cv::KeyPoint> &keypoints, vector<DescType> &desc) {
    for (auto &kp: keypoints) {
        ...
        for (int i = 0; i < 256; i++) {
            ...
            float cos_theta = std::cos(kp.angle*pi/180);
            float sin_theta = std::sin(kp.angle*pi/180);
            int u_p = cos_theta*ORB_pattern[4*i]-sin_theta*ORB_pattern[4*i+1]+kp.pt.x;
            int v_p = sin_theta*ORB_pattern[4*i]+cos_theta*ORB_pattern[4*i+1]+kp.pt.y;
            int u_q = cos_theta*ORB_pattern[4*i+2]-sin_theta*ORB_pattern[4*i+3]+kp.pt.x;
            int v_q = sin_theta*ORB_pattern[4*i+2]+cos_theta*ORB_pattern[4*i+3]+kp.pt.y;
            if(u_p>=0&&u_p<image.cols&&v_p>=0&&v_p<image.rows &&
                u_q>=0&&u_q<image.cols&&v_q>=0&&v_q<image.rows){
                d[i] = image.at<uchar>(v_p, u_p) > image.at<uchar>(v_q, u_q) ? 0 : 1;
            }
            else{
                d.clear();
                break;
...
void bfMatch(const vector<DescType> &desc1, const vector<DescType> &desc2, vector<cv::DMatch> &matches) {
    ...
    for(int i = 0; i < desc1.size(); ++i){
        ...
        if(bestDist<d_max){
            matches.push_back(cv::DMatch(i, bestj, bestDist));
        }
    }
    // END YOUR CODE HERE
